...
let yourSentence = "This \(nounArray[3]) is not working on my \(nounArray[1])."
let yourSillySentence = "This \(nounArray[index1]) can drink the \(nounArray[index2])."
//: ### Exercise 2
...
let howManyCharacters = "How much wood could a woodchuck chuck if a woodchuck could chuck wood?".count
//: ### Exercise 4
...
for letter in gString where letter == "g" || letter == "G" {count += 1}
//: ### Exercise 5
//: Write a program that tells you whether or not this string contains the substring "tuna".
let word = "fortunate"

func isContainsTuna(in baseString: String) -> Bool{
    return baseString.contains("tuna")
}

print(isContainsTuna(in: word))
//: ### Exercise 6
//: Write a program that deletes all occurrences of the word "like" in the following string.
let lottaLikes = "If like, you wanna learn Swift, like, you should build lots of small apps, cuz it's like, a good way to practice."

func iDontLikeLikesIn(sentence: String) -> String {
    var result = sentence
    let dontLikes = "like, "
    while(result.contains(dontLikes)) {
        result.removeSubrange(result.range(of: dontLikes)!)
    }
    return result
}
//: ### Exercise 7
...
for set in dictionary {
    newestString.replaceSubrange(newestString.range(of:set.key)!, with: set.value)
}
//: ### Exercise 8
...
//desired output = "$5.67"
let inDollar: String = "$" + "\(Double(numOfPennies)/100.0)"
//: # Let or Var?
import UIKit
import Foundation
//: ### Exercise 9
