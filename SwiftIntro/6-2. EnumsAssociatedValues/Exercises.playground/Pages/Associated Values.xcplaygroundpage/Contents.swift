...
let lunchFruit: [LunchFruit] = [.grapes(seedless: true), .clementines(count: 4), .apple(sliced: false)]
//: - Callout(Exercise):
...
enum Prize {
    case coffeeMug(image: UIImage)
    case businessCards(name: String, image: UIImage)
    case lanyard(image: UIImage, color: UIColor)
}
//: [Next](@next)
