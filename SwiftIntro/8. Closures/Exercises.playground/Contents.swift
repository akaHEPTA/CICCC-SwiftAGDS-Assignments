...
let divisibleByTwelve = numbersAsStrings.filter({(n: String) -> Bool in Int(n)! % 12 == 0 })
divisibleByTwelve
//: __3b.__
//: Rewrite the filtering closure expression to be as concise as possible.
let divisibleByTwelve2 = numbersAsStrings.filter{ Int($0)! % 12 == 0 }
//: __Problem 4__
//:
...
let newArray = particleSizesInMicrons.filter{ $0 < 20 }
newArray
//: __Problem 5__
//:
...
// Example
let sizesAsStrings: [String] = particleSizesInMicrons.map({ (size: Int) -> String in
    return "\(size) microns"
})
sizesAsStrings
...
let oneDollarInIndianRupees = 71.5

let pricesInDollars: [Int] = pricesInRupees.map{
    (price: Int) -> Int
    in let result: Double = (Double(price) / oneDollarInIndianRupees)
